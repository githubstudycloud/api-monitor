...
function extractTableDataWithStyles(table, maxColumns, settings) {
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    for (let c = 0; c < cells.length; c++) {
      ...
      if (colIndex >= maxColumns) continue; // 超出最大列数，忽略此单元格
      ...
      for (let ri = 0; ri < rowSpan; ri++) {
        for (let ci = 0; ci < colSpan; ci++) {
          if (rowIndex + ri < rows.length && colIndex + ci < maxColumns) {
            spanMatrix[rowIndex + ri][colIndex + ci] = true;
          }
        }
      }
...
function extractCellStyle(cell) {
  ...
  try {
    ...
    console.error('提取文本对齐方式时出错:', error);
  }
  ...
  try {
    ...
    console.error('提取垂直对齐方式时出错:', error);
  }
...
function mapVerticalAlign(cssVerticalAlign) {
  if (!cssVerticalAlign) return 'center';
...
function applyCellStyles(ws, styles) {
  ...
  styles.forEach(styleInfo => {
    if (!styleInfo || typeof styleInfo !== 'object') return;
    
    const { row, col, style } = styleInfo;
    if (typeof row !== 'number' || typeof col !== 'number' || !style) return;
    ...
    try {
      ...
      console.error('应用单元格样式时出错:', error);
    }
  });
}
...
async function exportToExcel(data, merges, styles, fileName = 'table-export', options = {}) {
  try {
    ...
    // 自动调整列宽
    autoAdjustColumnWidths(ws, actualData);
    
    // 自动调整行高
    autoAdjustRowHeights(ws, actualData);
    ...
    // 添加基础样式信息
    addStylesInfo(ws, actualData, options);
